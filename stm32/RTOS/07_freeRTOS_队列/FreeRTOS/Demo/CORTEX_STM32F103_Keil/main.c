...
int main(void)
{
	...
	InitUARTLock();
	xTaskCreate(Task1Function, "Task1", 1024, "hello task1", 1, &xHandleTask1); // 动态任务
	xTaskCreate(Task2Function, "Task2", 1024, "my name is liuxunzi", 1, &xHandleTask2);
	xTaskCreate(Task3Function, "Task3", 1024, NULL, 0, &xHandleTask3);
	// xHandleTask3 = xTaskCreateStatic(Task3Function, "Task3", 100, NULL, 1, xTask3Stack, &xTask3TCB);
	xTaskCreate(TaskGenericFunction, "Task4", 100, "Task4", 1, NULL); // 动态任务
	xTaskCreate(TaskGenericFunction, "Task5", 100, "Task5", 1, NULL);
	// Start the scheduler
	vTaskStartScheduler();
